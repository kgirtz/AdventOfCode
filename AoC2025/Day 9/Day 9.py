...
class Floor:
    def __init__(self, red_tiles: Sequence[XYpair]) -> None:
        self.red_tiles: list[XYpair] = list(red_tiles)# + [red_tiles[0]]  # For cyclical pairwise iteration
    ...
    def tile_intervals_in_column(self, x: int) -> list[tuple[int, int]]:
        ...
        for i, t1 in enumerate(self.red_tiles):
            ...
            if t1.x == t2.x == x:
                ...
                intersectors.append((min(t1.y, t2.y), max(t1.y, t2.y), is_s_curve))

            # Perpendicular, endpoints are covered by collinear case
            elif min(t1.x, t2.x) < x < max(t1.x, t2.x):
                intersectors.append((min(t1.y, t2.y), max(t1.y, t2.y), False))
    ...
    def tile_intervals_in_row(self, y: int) -> list[tuple[int, int]]:
        ...
        for i, t1 in enumerate(self.red_tiles):
            ...
            if t1.y == t2.y == y:
                t0: XYpair = self.red_tiles[(i - 1) % len(self.red_tiles)]
                t3: XYpair = self.red_tiles[(i + 2) % len(self.red_tiles)]
                is_s_curve: bool = (t1.y - t0.y) * (t1.y - t3.y) < 0
                intersectors.append((min(t1.x, t2.x), max(t1.x, t2.x), is_s_curve))

            # Perpendicular, endpoints are covered by collinear case
            elif min(t1.y, t2.y) < y < max(t1.y, t2.y):
                intersectors.append((min(t1.x, t2.x), max(t1.x, t2.x), False))
        ...
        for a, b in sides:
            if a.x == b.x:
                ...
                for i1, i2 in intervals:
                    if i1 <= min(a.y, b.y) and max(a.y, b.y) <= i2:
                        valid_sides.append(True)
                        break
            else:
                intervals = floor.tile_intervals_in_row(a.y)
                for i1, i2 in intervals:
                    if i1 <= min(a.x, b.x) and max(a.x, b.x) <= i2:
                        valid_sides.append(True)
                        break

        if len(valid_sides) == 4:
            return rectangle_size
            # max_size = max(max_size, size_of_rectangle(t1, t2))
